...
public class SanLianAllFragment extends Fast3ChildBaseFragment {
    @InjectView(R.id.rb_all_select)
    ReportButton rbAllSelect;

    @Override
    protected int initLayoutId() {
        return R.layout.fragment_sanlian_all;
    }
...
//    }
    ...
    public SelectedBallInfo selectOneByMachine() {
        // 通选
        SelectedBallInfo tongxuanSbiInfo = new SelectedBallInfo(getContext());
        tongxuanSbiInfo.setChangeFormat(false);
        tongxuanSbiInfo.hidePlusSymbol();
        tongxuanSbiInfo.addRedBallByString("三连号通选");
        tongxuanSbiInfo.setPerCost(gameRule.getR007());
        tongxuanSbiInfo.setBetMode(Constants.BET_MODE_SINGLE);
        tongxuanSbiInfo.setBetNum(1);
        return tongxuanSbiInfo;
    }
    ...
    public List<SelectedBallInfo> insertToSelectedArea() {
        ...
        if (rbAllSelect.isRbPressed()) {
            ...
            tongxuanSbiInfo.setBetMode(Constants.BET_MODE_SINGLE);
            tongxuanSbiInfo.setBetNum(1);
            infoArrayList.add(tongxuanSbiInfo);
        }
        resetWaitArea();
...
//            if (redNumList.size() == 1) {
            // 通选
            stringBuilder.append("200 200 200");
//            } else {
//                // 单式和复式
